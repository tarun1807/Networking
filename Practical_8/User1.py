...
if __name__ == '__main__':
    ...
    sender_window_size = int(input("Enter the sender window size : "))
    # connection = Connection()
    # connection.client_id = random.randint(1, 100)
    # connection.source_ip_address = "127.0.0.1"
    # connection.destination_ip_address = "127.0.01"
    list_of_packet_to_send = makeFragment(
        None, number_of_packet, connection, file_name_frag)

    main_data_file = open("my_message.txt", 'w')
    main_data_file.write(f"{file_name_frag} {number_of_packet}")
    main_data_file.close()

    main_data_file = open("pack.txt", 'w')
    main_data_file.write(f"{number_of_packet}")
    main_data_file.close()

    main_data_file = open("ssize.txt", 'w')
    main_data_file.write(f"{sender_window_size}")
    main_data_file.close()
    ...
    for i in range(0, len(list_of_packet_to_send)):
        file_name = ""
        file_name = "sender\\" + file_name_frag + "_num_" + str(i) + ".dat"
