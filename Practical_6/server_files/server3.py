...
class Server:
    ...
    def log(self, sender_ip, dest_ip, timestamp, payload_size):
        self.log_file.write(f"{sender_ip} | {dest_ip} | {timestamp} | {payload_size}\n")
        self.log_file.flush()
...
class receive(Thread):
    ...
    def run(self):
        while True:
            ...
            # # Get sender IP and destination IP
            sender_ip = client_add[0]
            # dest_ip = client_socket.recv(1024).decode()
            ...
            while True:
                ...
                if self.sc.verify(temp['Certificate']['Username'], temp['Certificate']['Password']) == "Certificate Verified successfully!":
                    ...
                    if temp["Packet ID"] == temp["Number of Packets"]-1:
                        ...
                        packets = []
                        flag = 1
                        dest_ip =receiver_ip #okok
                        # dest_ip = client_socket.recv(1024).decode()

                        
                        # Log packet information to file
                        timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                        payload_size = len(packet) * temp['Number of Packets']
                        # self.log(sender_ip, temp['Certificate']['Username'], dest_ip, timestamp, payload_size)
                        self.server.log(sender_ip,dest_ip, timestamp, payload_size)#ok
